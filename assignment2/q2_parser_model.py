...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.float32, shape = (None,self.config.n_classes), name= 'Target')
    ...
    def add_embedding(self):
        ...
        window = tf.nn.embedding_lookup(embedding_params, self.input_placeholder,partition_strategy="div")
        window = tf.reshape(
            window, (-1, self.config.n_features*self.config.embed_size)
        )
    ...
    def add_training_op(self, loss):
        ...
        """
        ### YOUR CODE HERE
        optim = tf.train.AdamOptimizer(self.config.lr)
